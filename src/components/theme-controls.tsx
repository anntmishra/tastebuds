...
import { rerollBlendPack, setBlendPack } from "@/app/actions";
import { Dice } from "@/components/icons";
import { PACKS, packName } from "@/lib/packs";

export function ThemeControls({ code, pack }: { code: string; pack: string }) {
  ...
  return (
    <div ref={box} className="relative flex items-center gap-2">
      ...
      <form action={rerollBlendPack.bind(null, code)}>
        <button
          type="submit"
      ...
      {open && (
        <div
          className="tb-card absolute right-0 top-full z-30 mt-2 w-[19rem] p-2.5"
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {PACKS.map((p) => (
              <form key={p.id} action={setBlendPack.bind(null, code, p.id)}>
                <button
                  type="submit"
                  title={p.note}
